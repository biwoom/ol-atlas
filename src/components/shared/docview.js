...
import { currentView, currentDocCardId, setCurrentDocCardId, getOrderedCardList, getPrevNextCard, updateHash, switchView, registerDocViewGuard, registerPostSwitchHook } from '../../core/router.js';
...
const _RM_KEY = 'ol_docview_readmode';
let _readMode = localStorage.getItem(_RM_KEY) === '1';
let _hoverTimers = {};

function setReadMode(on) {
  _readMode = on;
  localStorage.setItem(_RM_KEY, on ? '1' : '0');
  document.body.classList.toggle('dv-read-mode', on);
  _updateRmHoverListeners(on);
  _syncRmBtn();
}

function _syncRmBtn() {
  const btn = document.getElementById('dv-readmode-btn');
  if (!btn) return;
  btn.setAttribute('aria-pressed', _readMode ? 'true' : 'false');
  btn.title = _readMode ? '읽기 모드 종료' : '읽기 모드';
  if (_readMode) btn.classList.add('active');
  else btn.classList.remove('active');
}

function _initRmBtn() {
  if (document.getElementById('dv-readmode-btn')) return;
  const bar = document.querySelector('#view-document .view-bar');
  if (!bar) return;

  const btn = document.createElement('button');
  btn.id = 'dv-readmode-btn';
  btn.className = 'btn sm dv-readmode-btn';
  btn.setAttribute('aria-pressed', 'false');
  // 책 아이콘
  btn.innerHTML =
    `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" ` +
    `stroke-linecap="round" stroke-linejoin="round" style="width:1rem;height:1rem;flex-shrink:0">` +
    `<path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/>` +
    `<path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>` +
    `</svg>`;
  btn.onclick = () => setReadMode(!_readMode);

  const editBtn = document.getElementById('dv-edit-btn');
  if (editBtn && editBtn.parentNode === bar) {
...
function _updateRmHoverListeners(on) {
  if (on) {
    document.addEventListener('mousemove', _handleRmMouseMove);
    document.addEventListener('touchstart', _handleRmTouchStart, { passive: true });
  } else {
    document.removeEventListener('mousemove', _handleRmMouseMove);
    document.removeEventListener('touchstart', _handleRmTouchStart);
...
function _handleRmMouseMove(e) {
  ...
  if (e.clientY < 60) {
    ...
    _hoverTimers.header = setTimeout(() => {
      if (!header?.matches(':hover')) header?.classList.remove('peek');
    }, 1500);
  }
  ...
  if (e.clientX < 40) {
    ...
    _hoverTimers.sidebar = setTimeout(() => {
      if (!sidebar?.matches(':hover')) sidebar?.classList.remove('peek');
    }, 1500);
  }
}

function _handleRmTouchStart(e) {
...
// 뷰 전환 시 dv-read-mode 클래스 on/off
registerPostSwitchHook(v => {
  if (v === 'document') {
    _initRmBtn();
    if (_readMode) {
      document.body.classList.add('dv-read-mode');
      _updateRmHoverListeners(true);
    }
  } else {
    document.body.classList.remove('dv-read-mode');
    _updateRmHoverListeners(false);
  }
});
